...
export default function Employee() {
    ...
    const fetchEmployees = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://eshwar-backend.azurewebsites.net/api/v1/employeeManager/employees');
            if (!response.ok) {
    ...
    const handleDeleteEmployee = async (employeeId) => {
        try {
            const response = await fetch(`https://eshwar-backend.azurewebsites.net/api/v1/employeeManager/employees/${employeeId}`, {
                method: 'DELETE',
            });
            if (response.ok) {
                fetchEmployees();
